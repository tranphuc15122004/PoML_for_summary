...
| Total steps | 800 | 800 | |
| Dtype | bf16 | bf16 | |
| Repetition penalty | **1.0** | 1.0 | PHẢI giữ 1.0 — 1.3 phá huỷ R_acc (penalise vocab bài gốc) |
| No-repeat ngram size | **0** | 0 | PHẢI giữ 0 — ngram=3 cấm dùng lại trigram từ source |
...
```
Range [0, 1]. LCS tính ở mức syllable (split by space).

**R_len — Length Adherence:**

| Loại constraint | Logic |
|---|---|
...
| Job ID | Variant | Output dir | Steps | Ghi chú |
|---|---|---|---|---|
...
**Chẩn đoán:** `repetition_penalty > 1.0` và `no_repeat_ngram_size > 0` áp dụng lên toàn bộ `input_ids` (kể cả prompt ~2000 tokens). Model bị phạt khi reuse vocab từ bài gốc → summary không thể dùng từ ngữ bài báo → ROUGE-L = 0. Đây là **lỗi thiết kế của HuggingFace generate()**, không phải lỗi model. Hai tham số này phải giữ ở giá trị mặc định (1.0 và 0).
...
| Job ID | Variant | Output dir | R_acc trung bình | KL (step 200) |
|---|---|---|---|---|
...
3. `beta=0.15` — KL penalty kìm policy quá mạnh (KL thực chỉ 0.005, không cần 0.15)

---

### 6.8 Qwen3-4B — GRPO v5 (config tối ưu, mới submit)

Submit 21/06/2026 với config điều chỉnh dựa trên phân tích v4:

| Job ID | Variant | Output dir | Config thay đổi |
|---|---|---|---|
| 171836659 | base + fresh | `grpo_qwen3_4b_base_fresh_v5` | G=8, LR=2e-6, beta=0.04 |
| 171836660 | base + sft | `grpo_qwen3_4b_base_sft_v5` | G=8, LR=2e-6, beta=0.04 |
| 171836662 | instruct + fresh | `grpo_qwen3_4b_instruct_fresh_v5` | G=8, LR=2e-6, beta=0.04 |
| 171836663 | instruct + sft | `grpo_qwen3_4b_instruct_sft_v5` | G=8, LR=2e-6, beta=0.04 |

**Lý do thay đổi:**
- `G: 4→8` — giảm variance advantage ~40% (std ∝ 1/√G), gradient signal rõ hơn
- `LR: 5e-7→2e-6` — 4× lớn hơn, vẫn conservative với GRPO LoRA
- `beta: 0.15→0.04` — KL penalty phù hợp với mức KL thực tế; 0.15 trong v4 kìm không cần thiết

**Walltime:** 24h (với G=8, ước ~55s/step → 800 steps ≈ 12.2h, vẫn trong walltime).

**PBS script cập nhật:** Thêm `TEMPERATURE` env var để có thể override qua `qsub -v`.
...
| Model | ROUGE-1 | ROUGE-2 | ROUGE-L | Length Hit Rate | Avg Gen Length | BARTScore | G-Eval avg |
|---|---|---|---|---|---|---|---|
...
|---|---|---|
| flash_attn 2.7.4 (pre-installed) | ❌ ImportError | C++ ABI mismatch: old `Ss` vs new `__cxx11` string; CUDA symbol `...ib` → `...jb` |
...
1. **Theo dõi v5 jobs** (171836659–663, queue hiện tại)
   - Dấu hiệu học tốt sau step 50–100:
     - KL tăng lên >0.01 (policy đang diverge tích cực)
     - R_acc slope dương rõ ràng (>0.001/step)
     - Signal/Noise ratio R_acc > 0.3
   - Nếu v5 vẫn flat: thử `LR=5e-6` hoặc tăng `num_generations=12`

2. **Dọn dẹp thư mục models**
   - Cần xóa: `grpo_qwen3_4b_{base,instruct}_{fresh,sft}_v3` (4 dirs × ~3.7G ≈ 15G)
   - Cần xóa: `grpo_validate_fix` (1.5G, debug run)
   - Giữ: v4 (đang chạy), v5 (mới submit), SFT finals

3. **Chạy eval so sánh** sau khi v4 hoàn thành:
   ```bash
   qsub scripts/pbs/eval.pbs
   ```
   Metrics mục tiêu: ROUGE-L, Length Hit Rate, BARTScore

### Khi v5 hoàn thành

4. **So sánh v4 vs v5** — xác nhận G=8 + LR=2e-6 + beta=0.04 cải thiện learning curve
5. **Nếu v5 học tốt:** kích hoạt `length_advantage_alpha > 0` (hiện tại =0.0) để amplify completion đúng độ dài
