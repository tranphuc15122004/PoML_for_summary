...
| **VLSP 2022 AbMuSu** | multi-document, **extractive** | ~285 train / 15 val | Chỉ dùng cho GRPO (nhãn extractive, không phù hợp làm target cho SFT) |
...
>
> - **VLSP**: Không xuất hiện trong SFT (cả train lẫn val) vì VLSP có nhãn **extractive** — target là các câu được chọn từ văn bản gốc, không phải bản tóm tắt trừu tượng (abstractive). SFT cần target abstractive chất lượng cao; dùng VLSP làm validation sẽ đánh giá sai năng lực tóm tắt thực sự (metric so sánh với target extractive).
...
2. **VLSP chỉ dùng cho GRPO (285 mẫu):**  
   VLSP là tập extractive — target là các câu được chọn từ văn bản gốc, không phải tóm tắt trừu tượng (abstractive). Do đó không phù hợp làm gold summary cho SFT (học bắt chước đầu ra có sẵn). GRPO vẫn dùng được vì reward đánh giá chất lượng tóm tắt do model sinh ra, không yêu cầu target phải là abstractive hoàn chỉnh.
...
- **Single-document (WikiLingua, VietNews):** Các biến thể Qwen3-4B sau post-training (SFT+GRPO) đạt 23–30%, **vượt tất cả mô hình tham khảo cùng cỡ** (Qwen3-4B base: 19–21%) và cạnh tranh với các mô hình lớn hơn nhiều (Qwen3-14B: 18–20%, Llama3.3-70B: 19–22%, GPT-4o: 20–22%). Chỉ VietAI (mô hình chuyên biệt tóm tắt) là còn dẫn đầu (33–34%).

- **Multi-document (ViMs, VLSP):** Kết quả trái ngược nhau. Trên ViMs, các mô hình lớn (Qwen3-14B: 44%, GPT-4o: 44%) vượt trội so với 4B (max 26%). Trên VLSP, `QWEN3_BASE_sft` đạt 28.14% — cao hơn gpt-3.5-turbo (35.79%) và cạnh tranh với các mô hình 14B+ (~40–44%). Tuy nhiên, GRPO trên VLSP lại không cải thiện được so với SFT thuần.
...
- **Single-document — vượt trội tuyệt đối:** Trên VietNews và WikiLingua, các biến thể SFT/GRPO của Qwen3-4B đạt LenDist cực thấp (1.0–14.2), **tốt hơn mọi mô hình tham khảo** bao gồm GPT-4o (8–11) và Llama3.3-70B (13–14). Đây là thành công trực tiếp của việc huấn luyện với length reward trong GRPO kết hợp với SFT.
...
1. **Single-document: cạnh tranh mạnh với mô hình lớn.** Dù chỉ có 4B tham số, pipeline SFT+GRPO tạo ra chất lượng tóm tắt (ROUGE-2) vượt Qwen3-14B, Llama3.3-70B và GPT-4o trên WikiLingua và VietNews. Khả năng tuân thủ độ dài (LenDist) **vượt trội hoàn toàn** tất cả tham khảo.
